...
class Linear_Region_Collector:
    ...
    def register_hook(self, model):
        for m in model.modules():
            ...
            #if isinstance(m, ReLUMax):
            if isinstance(m, nn.Hardswish) or isinstance(m, ReLUMax) or isinstance(m, nn.ReLU):
...
def compute_TE_NAS_score(gpu, model, resolution, batch_size):
    ntk = compute_NTK_score(gpu=gpu, model=model,
